...
@implementation PushTokenWebService
...
- (void)updateDevicePushTokenWithToken:(NSString *)authToken andPushToken:(NSString *)devicePushToken {
    NSString *stringData = [NSString stringWithFormat:@"{\"access_token\":\"%@\", \"device_token\": \"%@\"}", authToken, devicePushToken];
    NSData *jsonData = [stringData dataUsingEncoding:NSUTF8StringEncoding allowLossyConversion:YES];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:@"http://oskofeya.com/api/update_device_token"] cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:30.0];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    ...
    [self.pushTokenWsDelegate pushTokenWebService:self errorMessage:error.localizedDescription];
}
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    [self.pushTokenWsDelegate pushTokenWebService:self returnMessage:returnMsg];
}
